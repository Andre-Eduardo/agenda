...
function KV({label, value, mono, emptyText = '—', className, ref, ...props}: KVProps) {
    ...
    return (
        <div ref={ref} className={clsx(styles.root, className)} {...props}>
            ...
            <dd className={clsx(styles.value, mono && styles.valueMono, isEmpty && styles.valueEmpty)}>
...
function KVGrid({cols = 2, className, ref, ...props}: React.ComponentProps<'dl'> & {cols?: 2 | 3 | 4}) {
    return <dl ref={ref} className={clsx(styles.grid, colsClass[cols], className)} {...props} />;
}
