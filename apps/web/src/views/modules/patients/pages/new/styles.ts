...
export const aiNudge = {
    root: cn(
        'mt-[18px] flex items-start gap-[10px] border-l-[3px] border-(--color-ai-border) bg-(--color-ai-bg) px-[14px] py-3'
    ),
    icon: cn(
        'inline-flex size-7 shrink-0 items-center justify-center rounded-[6px] bg-(--color-ai-badge-bg) text-white'
    ),
    body: cn('flex-1'),
    title: cn('flex items-center gap-[6px] text-[13px] font-medium leading-[1.3] text-(--color-ai-text)'),
    badge: cn(
        'rounded-[4px] bg-(--color-ai-badge-bg) px-[5px] py-px text-[10px] font-medium tracking-[0.06em] text-white'
    ),
    sub: cn('mt-0.5 text-xs leading-[1.5] text-(--color-ai-text) opacity-85'),
    btn: cn(
        'inline-flex shrink-0 items-center gap-[5px] rounded-[6px] bg-(--color-ai-badge-bg) px-[11px] py-1.5 text-xs font-medium text-white transition-colors hover:bg-(--color-ai-border)'
    ),
};

// Contador numérico nos triggers de tab (usado dentro de TabsTrigger)
export const tabNum = cn(
    'inline-flex size-[22px] shrink-0 items-center justify-center rounded-full border border-(--color-border) bg-(--color-bg-card) text-[11px] font-semibold tabular-nums text-(--color-text-secondary)'
);
...
export const photo = {
    ...
    overlay: cn(
        'absolute inset-0 flex flex-col items-center justify-center gap-1 bg-black/55 text-[11px] font-medium text-white opacity-0 transition-opacity duration-(--duration-fast) ease-out group-hover:opacity-100'
    ),
...
export const subSection = {
    ...
    tag: cn(
        'rounded-[4px] border border-(--color-border) bg-(--color-bg-card) px-[7px] py-0.5 text-[11px] text-(--color-text-tertiary)'
    ),
...
export const footer = {
    ...
    step: cn(
        'ml-1 rounded-full border border-(--color-border) bg-(--color-bg-surface) px-[10px] py-1 text-[11px] font-medium tracking-[0.02em] text-(--color-text-secondary)'
    ),
