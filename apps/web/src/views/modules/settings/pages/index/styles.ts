...
export const sideNavTitle = cn(
    'text-[11px] font-medium text-(--color-text-tertiary) uppercase tracking-wider px-2 mb-2'
);
...
// Tabs
export const tabList = cn('flex border-b border-(--color-border) bg-(--color-bg-surface) px-5 gap-1 overflow-x-auto');
...
export const tabNum = cva('flex items-center justify-center w-5 h-5 rounded-full text-[11px] font-medium shrink-0', {
    variants: {
        active: {
...
export const pwdRevealBtn = cn(
    'absolute right-3 top-1/2 -translate-y-1/2 text-(--color-text-tertiary) hover:text-(--color-text-secondary)'
);
...
export const photoInitials = cn('text-2xl font-medium text-(--color-text-tertiary)');
...
export const sessionIcon = cn(
    'w-9 h-9 flex items-center justify-center rounded-[6px] bg-(--color-bg-card) text-(--color-text-secondary) shrink-0'
);
