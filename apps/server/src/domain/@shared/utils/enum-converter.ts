...
export function toEnum<T extends EnumLike>(enumType: T, value: string): T[keyof T] {
    ...
    if (!validValues.includes(value)) {
        throw new InvalidInputException(`Invalid enum value "${value}". Expected one of: ${validValues.join(', ')}.`);
    }
 ...
 */
export function toEnumOrNull<T extends EnumLike>(enumType: T, value: string | null | undefined): T[keyof T] | null {
