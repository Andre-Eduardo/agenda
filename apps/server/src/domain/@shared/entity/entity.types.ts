...
import type { Entity } from "@domain/@shared/entity/entity.base";
...
export type AllEntityProps<T extends Entity<Identifier<string>>> = FinalType<
  {
    [K in Exclude<NonFunctionPropertyNames<T>, "events"> as K extends "deletedAt"
      ? never
      : K]: T[K];
  } & {
    deletedAt?: Date | null;
...
export type EntityProps<T extends Entity<Identifier<string>>> = Omit<
  AllEntityProps<T>,
  EntityProperties
>;

export type CreateEntity<T extends Entity<Identifier<string>>> = Nullish<
  Omit<AllEntityProps<T>, EntityProperties>
>;
