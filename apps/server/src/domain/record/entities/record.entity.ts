...
import {
  ...
  RecordSignedEvent,
  RecordReopenedEvent,
} from "@domain/record/events";
import type { File } from "@domain/record/entities/file.entity";
...
export class Record extends AggregateRoot<RecordId> {
  ...
  sign(memberId: ClinicMemberId): void {
    ...
    this.updatedAt = now;
    this.addEvent(
      new RecordSignedEvent({ recordId: this.id, signedByMemberId: memberId, timestamp: now }),
    );
  }
  ...
  unlock(requestedByMemberId: ClinicMemberId): void {
    ...
    this.updatedAt = now;
    this.addEvent(
      new RecordReopenedEvent({ recordId: this.id, requestedByMemberId, timestamp: now }),
    );
  }
