...
} from '../../record/entities/record.entity';
import {DraftApprovedEvent} from '../events/draft-approved.event';
...
export class DraftEvolution extends AggregateRoot<DraftEvolutionId> {
    ...
    updateFields(
        fields: Partial<Pick<DraftEvolution,
            | 'templateType' | 'title' | 'attendanceType' | 'clinicalStatus'
            | 'conductTags' | 'subjective' | 'objective' | 'assessment'
            | 'plan' | 'freeNotes' | 'eventDate'
        >>
    ): void {
        if (this.status === DraftEvolutionStatus.APPROVED || this.status === DraftEvolutionStatus.REJECTED) {
    ...
    approve(memberId: ClinicMemberId, recordId: RecordId): void {
        ...
        this.addEvent(new DraftApprovedEvent({
            draftId: this.id,
            recordId,
