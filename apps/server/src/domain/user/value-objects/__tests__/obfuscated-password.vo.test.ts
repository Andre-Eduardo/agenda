...
describe("A value object representing an obfuscated password", () => {
  ...
  it("should fail when decoding invalid string", () => {
    expect(() => ObfuscatedPassword.decode("foo")).toThrowWithMessage(
      Error,
      "Invalid obfuscated password format.",
    );
  });
  ...
  it("should fail to obfuscate when node crypto fails", async () => {
    ...
    const { ObfuscatedPassword: LocalObfuscatedPassword } =
      await import("../obfuscated-password.vo");
  ...
  it("should fail to verify a password when node crypto fails", async () => {
    ...
    await expect(decoded.verify("4SecureP@ssword")).rejects.toThrowWithMessage(
      Error,
      "Unknown error",
    );
  });
