...
import {CreateFormTemplateVersionDto, FormTemplateVersionDto} from '../dtos';

export type CreateVersionPayload = CreateFormTemplateVersionDto & {templateId: FormTemplateId};

@Injectable()
export class CreateFormTemplateVersionService
    implements ApplicationService<CreateVersionPayload, FormTemplateVersionDto>
{
    constructor(
    ...
    async execute({payload}: Command<CreateVersionPayload>): Promise<FormTemplateVersionDto> {
        ...
        const nextVersionNumber = await this.formTemplateVersionRepository.findNextVersionNumber(template.id);
