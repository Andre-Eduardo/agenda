...
export class BillingReportService {
    ...
    async getMemberCostReport(
        ...
        const avgCostPerMessageUsd = interactionCount > 0 ? Number((totalCostUsd / interactionCount).toFixed(8)) : 0;

        const modelBreakdown: ModelBreakdownEntry[] = [...byModel.entries()].map(([modelId, data]) => ({
            modelId,
    ...
    async getClinicCostReport(clinicId: string, year: number, month: number): Promise<ClinicCostReport> {
        ...
        const totalAiCostUsd = Number(members.reduce((sum, m) => sum + m.aiCost.totalCostUsd, 0).toFixed(8));
        const totalAiCostBrl = usdToBrl(totalAiCostUsd);
        const avgMarginPercent =
            members.length > 0
                ? Number((members.reduce((sum, m) => sum + m.margin.grossMarginPercent, 0) / members.length).toFixed(2))
                : 0;
