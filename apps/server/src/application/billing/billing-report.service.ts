...
export class BillingReportService {
  ...
  async getMemberCostReport(
    ...
    const avgCostPerMessageUsd =
      interactionCount > 0 ? Number((totalCostUsd / interactionCount).toFixed(8)) : 0;
  ...
  async getClinicCostReport(
    clinicId: string,
    year: number,
    month: number,
  ): Promise<ClinicCostReport> {
    this.validatePeriod(year, month);
    ...
    const avgMarginPercent =
      members.length > 0
        ? Number(
            (
              members.reduce((sum, m) => sum + m.margin.grossMarginPercent, 0) / members.length
            ).toFixed(2),
          )
        : 0;
