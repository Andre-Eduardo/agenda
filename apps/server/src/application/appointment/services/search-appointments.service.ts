...
> {
  ...
  async execute({
    actor,
    payload,
  }: Command<SearchAppointmentsDto>): Promise<PaginatedDto<AppointmentDto>> {
    ...
    const paymentByAppointmentId = new Map(
      payments.map((p) => [p.appointmentId.toString(), p.status]),
    );
    ...
    return {
      data: result.data.map(
        (appointment) =>
          new AppointmentDto(
            appointment,
            paymentByAppointmentId.get(appointment.id.toString()) ?? null,
          ),
      ),
