...
import { ClinicalProfileDto, UpsertClinicalProfileDto } from "@application/clinical-profile/dtos";

@Injectable()
export class UpsertClinicalProfileService implements ApplicationService<
  UpsertClinicalProfileDto,
  ClinicalProfileDto
> {
  constructor(
  ...
  async execute({
    actor,
    payload,
  }: Command<UpsertClinicalProfileDto>): Promise<ClinicalProfileDto> {
