...
export class KnowledgeDebugController {
    constructor(
        private readonly retrieveService: RetrieveKnowledgeChunksService,
    ) {}
    ...
    async searchKnowledge(@Body() dto: SearchKnowledgeDto) {
        if (process.env['NODE_ENV'] !== 'development' || process.env['AGENT_ENABLE_KNOWLEDGE_DEBUG'] !== 'true') {
            throw new ForbiddenException('This endpoint is only available in development mode with AGENT_ENABLE_KNOWLEDGE_DEBUG=true.');
        }
