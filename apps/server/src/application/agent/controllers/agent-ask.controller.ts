...
import type { AgentResponse } from "@application/agent/interfaces";
...
const agentAskSchema = z.object({
  ...
  patientId: z
    ...
    .uuid()
    .optional()
    .describe("Optional patient UUID for patient-scoped context."),
...
export class AgentAskController {
  ...
  @ApiOperation({
    ...
    responses: [
      { status: 200, description: "Agent response with answer and tool call audit trail" },
    ],
  })
  @Authorize(RecordPermission.VIEW)
  @HttpCode(200)
  @Post("ask")
  async ask(@RequestActor() actor: Actor, @Body() dto: AgentAskDto): Promise<AgentResponse> {
