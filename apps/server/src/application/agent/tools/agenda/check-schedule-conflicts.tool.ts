...
const schema = z.object({
    ...
    excludeAppointmentId: z
        ...
        .uuid()
        .optional()
        .describe('Appointment ID to exclude from conflict check (useful when rescheduling).'),
});
...
export class CheckScheduleConflictsTool implements AgentTool<Input, Output> {
    ...
    async execute(input: Input, context: ToolContext): Promise<Output> {
        ...
        const excludeId = input.excludeAppointmentId ? AppointmentId.from(input.excludeAppointmentId) : undefined;
        ...
        return {
            ...
            conflicts: conflicts.map((c) => toAppointmentView(c)),
        };
    }
}
