...
import type {PatientView} from '@application/agent/mappers/patient-view.mapper';
import {toPatientView} from '@application/agent/mappers/patient-view.mapper';
...
export class GetPatientTool implements AgentTool<Input, Output> {
    ...
    async execute(input: Input, context: ToolContext): Promise<Output> {
        const patient = await this.patientRepository.findById(PatientId.from(input.patientId), context.clinicId);
