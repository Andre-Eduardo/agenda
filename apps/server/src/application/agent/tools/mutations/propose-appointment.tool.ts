...
import {
  ...
  type AppointmentProposalResult,
} from "@application/agent-proposal/services/create-appointment-proposal.service";
...
const schema = z.object({
  ...
  type: z
    .enum(["FIRST_VISIT", "FOLLOW_UP", "EVALUATION", "PROCEDURE", "TELEMEDICINE", "INTERCURRENCE"])
    .optional(),
...
export class ProposeAppointmentTool implements AgentTool<Input, AppointmentProposalResult> {
  ...
  execute(input: Input, context: ToolContext): Promise<AppointmentProposalResult> {
