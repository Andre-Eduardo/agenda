...
import {
  ...
  type PatientAlertProposalResult,
} from "@application/agent-proposal/services/create-patient-alert-proposal.service";
...
const schema = z.object({
  ...
  title: z
    ...
    .min(3)
    .max(255)
    .describe('Short title for the alert (e.g. "Penicillin allergy").'),
...
export class ProposePatientAlertTool implements AgentTool<Input, PatientAlertProposalResult> {
  ...
  execute(input: Input, context: ToolContext): Promise<PatientAlertProposalResult> {
