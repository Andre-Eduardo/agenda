...
describe('A password schema', () => {
    it.each(['Pa$$w0rd', '4BIG_password_here_with_symbol_and_number_1!'])('should accept valid payloads', (payload) => {
    ...
    it('should reject when failing to parse', () => {
        jest.spyOn(ObfuscatedPassword, 'validate').mockImplementationOnce(() => {
            // eslint-disable-next-line no-throw-literal -- For testing purposes
            throw 'Unknown error';
        });
