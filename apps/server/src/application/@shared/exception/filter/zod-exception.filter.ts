...
import { getViolations } from "@application/@shared/exception/zod.exception";
...
export class ZodExceptionFilter implements ExceptionFilter {
  constructor(private readonly httpAdapterHost: HttpAdapterHost<ExpressAdapter>) {}
    ...
    const problemDetails = formatter.invalidInput(
      "The payload sent is not valid",
      getViolations(exception),
    );
