...
export class RequestContextMiddleware implements NestMiddleware {
  ...
  use(request: Request, response: Response, next: NextFunction): void {
    ...
    if (request.method === "GET") {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access -- request.body is typed as any in Express
      request.body.clinicMemberId ??= this.getClinicMemberFromRequest(request);
    }

    response.actions = new ExpressContextActions(
      response,
      this.authCookie,
      this.clinicMemberCookie,
      {
        ...this.cookieOptions,
