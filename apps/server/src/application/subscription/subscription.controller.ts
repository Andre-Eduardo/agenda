...
import {Authorize, Public} from '@application/@shared/auth';
...
import {
    AddonCatalogItemDto,
    MemberActiveAddonsDto,
    buildAddonCatalogItems,
} from '@application/subscription/dto/addon.dto';
import {ChangePlanDto} from '@application/subscription/dto/change-plan.dto';
import {PurchaseAddonDto} from '@application/subscription/dto/purchase-addon.dto';
import {SubscriptionDto} from '@application/subscription/dto/subscription.dto';
import {
    ActiveAddonDto,
    ClinicMembersUsageSummaryDto,
    CurrentUsageDto,
    MemberUsageSummaryDto,
} from '@application/subscription/dto/usage.dto';
import {AddonCode, PlanCodeRecord} from '@application/subscription/subscription-plans.config';
...
export class SubscriptionController {
    ...
    async getMemberUsage(@RequestActor() actor: Actor, @Param('memberId') memberId: string): Promise<CurrentUsageDto> {
        const usage = await this.subscriptionService.getCurrentUsage(memberId, actor.clinicId.toString());
    ...
    @ApiOperation({
        ...
        responses: [
            {
                status: 201,
                description: 'Add-on purchased; returns updated usage',
    ...
    async getMemberAddons(
        ...
        const addons = await this.subscriptionService.getActiveAddonDetails(memberId, actor.clinicId.toString());
    ...
    @ApiOperation({
        ...
        responses: [
            {
                status: 200,
                description: 'Clinic-wide usage breakdown',
                type: ClinicMembersUsageSummaryDto,
            },
        ],
    })
    ...
    @ApiOperation({
        ...
        parameters: [entityIdParam('Clinic ID', 'clinicId')],
        // eslint-disable-next-line @typescript-eslint/no-deprecated -- backward compat endpoint
