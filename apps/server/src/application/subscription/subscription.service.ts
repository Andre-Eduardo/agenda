...
export class SubscriptionService {
  ...
  async getOrCreateUsageRecord(memberId: string, clinicId: string) {
    ...
    return this.prisma.usageRecord.upsert({
      where: {
        usage_record_member_period_unique: { memberId, periodYear: year, periodMonth: month },
      },
      create: {
        id: randomUUID(),
        clinicId,
  ...
  async incrementUsage(memberId: string, clinicId: string, metric: UsageMetric, amount = 1) {
    ...
    if (metric === "docs") {
      data = { docsUploaded: { increment: amount }, updatedAt: new Date() };
    } else if (metric === "chat") {
      data = { chatMessages: { increment: amount }, updatedAt: new Date() };
    } else {
      data = { clinicalImages: { increment: amount }, updatedAt: new Date() };
    }
  ...
  async getCurrentUsage(memberId: string, clinicId: string): Promise<CurrentUsageResult> {
    ...
    if (docs.limit !== null && docs.remaining === 0 && docs.status !== "NOT_INCLUDED")
      limitsReached.push("docs");

    if (chat.limit !== null && chat.remaining === 0 && chat.status !== "NOT_INCLUDED")
      limitsReached.push("chat");

    if (images.limit !== null && images.remaining === 0 && images.status !== "NOT_INCLUDED")
      limitsReached.push("images");

    if (
      storageHotGb.limit !== null &&
      storageHotGb.remaining === 0 &&
      storageHotGb.status !== "NOT_INCLUDED"
    )
      limitsReached.push("storageHotGb");
    ...
    const addonDetails: AddonDetail[] = activeAddons.map(({ addonCode, quantity }) => {
      ...
      if (catalog.grants.docsPerMonth !== undefined)
        grantsTotal.docsPerMonth = catalog.grants.docsPerMonth * quantity;

      if (catalog.grants.chatMessagesPerMonth !== undefined)
        grantsTotal.chatMessagesPerMonth = catalog.grants.chatMessagesPerMonth * quantity;

      if (catalog.grants.clinicalImagesPerMonth !== undefined)
        grantsTotal.clinicalImagesPerMonth = catalog.grants.clinicalImagesPerMonth * quantity;

      if (catalog.grants.storageHotGb !== undefined)
        grantsTotal.storageHotGb = catalog.grants.storageHotGb * quantity;
  ...
  async purchaseAddon(
    ...
    const existing = await this.prisma.subscriptionAddon.findUnique({
      where: {
        subscription_addon_member_code_period_unique: {
          memberId,
          addonCode,
          periodYear: year,
          periodMonth: month,
    ...
    await (existing
      ? this.prisma.subscriptionAddon.update({
          where: { id: existing.id },
          data: {
            quantity: existing.quantity + quantity,
            pricePaidBrl: existing.pricePaidBrl + pricePaidBrl,
            updatedAt: now,
          },
        })
      : this.prisma.subscriptionAddon.create({
          data: {
            id: randomUUID(),
            clinicId,
            memberId,
  ...
  async getActiveAddonDetails(memberId: string, _clinicId: string): Promise<AddonDetail[]> {
    ...
    return rows
      ...
      .map((r) => {
        ...
        if (catalog.grants.docsPerMonth !== undefined)
          grantsTotal.docsPerMonth = catalog.grants.docsPerMonth * r.quantity;

        if (catalog.grants.chatMessagesPerMonth !== undefined)
          grantsTotal.chatMessagesPerMonth = catalog.grants.chatMessagesPerMonth * r.quantity;

        if (catalog.grants.clinicalImagesPerMonth !== undefined)
          grantsTotal.clinicalImagesPerMonth = catalog.grants.clinicalImagesPerMonth * r.quantity;

        if (catalog.grants.storageHotGb !== undefined)
          grantsTotal.storageHotGb = catalog.grants.storageHotGb * r.quantity;
   ...
   */
  async getClinicActiveAddons(
    clinicId: string,
  ): Promise<Array<{ memberId: string; addons: AddonDetail[] }>> {
    const { year, month } = this.currentPeriod();
    const expiresAt = this.periodEndIso(year, month);
    ...
    for (const row of rows) {
      ...
      if (catalog.grants.docsPerMonth !== undefined)
        grantsTotal.docsPerMonth = catalog.grants.docsPerMonth * row.quantity;

      if (catalog.grants.chatMessagesPerMonth !== undefined)
        grantsTotal.chatMessagesPerMonth = catalog.grants.chatMessagesPerMonth * row.quantity;

      if (catalog.grants.clinicalImagesPerMonth !== undefined)
        grantsTotal.clinicalImagesPerMonth = catalog.grants.clinicalImagesPerMonth * row.quantity;

      if (catalog.grants.storageHotGb !== undefined)
        grantsTotal.storageHotGb = catalog.grants.storageHotGb * row.quantity;
