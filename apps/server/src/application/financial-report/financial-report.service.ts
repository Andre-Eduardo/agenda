...
export class FinancialReportService {
  ...
  async getRevenueSummary(
    clinicId: string,
    actorMemberId: string,
    filters: Omit<RevenueReportFilters, "page" | "pageSize">,
  ...
  private computeSummary(payments: Array<{ amountBrl: number; status: string }>): RevenueSummary {
    ...
    for (const p of payments) {
      ...
      if (p.status === "PAID") totalPaid += p.amountBrl;
      else if (p.status === "PENDING") totalPending += p.amountBrl;
      else if (p.status === "EXEMPT") totalExempt += p.amountBrl;
      else if (p.status === "REFUNDED") totalRefunded += p.amountBrl;
    }

    const appointmentCount = payments.length;
    const avgTicketBrl =
      appointmentCount > 0 ? Number((totalBrl / appointmentCount).toFixed(2)) : 0;
  ...
  private groupByPaymentMethod(
    payments: Array<{ amountBrl: number; paymentMethod: string }>,
  ...
  private groupByProfessional(
    payments: Array<{
      ...
      appointment: {
        ...
        attendedBy: { displayName: string | null; user: { name: string } };
      };
    }>,
  ): ProfessionalSummary[] {
    const byProfessional = new Map<
      string,
      { displayName: string; count: number; totalBrl: number }
    >();

    for (const p of payments) {
      const memberId = p.appointment.attendedByMemberId;
      const displayName =
        p.appointment.attendedBy.displayName ?? p.appointment.attendedBy.user.name;
