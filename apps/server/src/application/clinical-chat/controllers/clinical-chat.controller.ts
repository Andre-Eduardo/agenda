...
} from "@application/clinical-chat/dtos";
import {
...
export class ClinicalChatController {
  ...
  @ApiOperation({
    ...
    responses: [
      { status: 200, description: "Agent profiles list", type: AiAgentProfileDto, isArray: true },
    ],
  })
  ...
  @ApiOperation({
    ...
    responses: [
      {
        status: 200,
        description: "User message + assistant response",
  ...
  async listMessages(
    @RequestActor() actor: Actor,
    @ValidatedParam("id", sessionIdSchema.shape.id) id: PatientChatSessionId,
    @Query(new ZodValidationPipe(listChatMessagesSchema.omit({ sessionId: true }))) query: Omit<
      ListChatMessagesDto,
      "sessionId"
    >,
  ...
  @ApiOperation({
    ...
    responses: [
      { status: 200, description: "Snapshot retrieved", type: PatientContextSnapshotDto },
    ],
  })
  @Authorize(ClinicalChatPermission.VIEW)
  ...
  async getSnapshot(
    ...
    @ValidatedParam(
      ...
      z
        .string()
        .uuid()
        .transform((v) => PatientId.from(v)),
    )
    patientId: PatientId,
  ): Promise<{
    snapshot: PatientContextSnapshotDto | null;
    wasRebuilt: boolean;
    isStale: boolean;
  }> {
  ...
  @ApiOperation({
    ...
    parameters: [entityIdParam("Session ID")],
    responses: [
      { status: 200, description: "Session context state", type: SessionContextInspectDto },
    ],
  })
  @Authorize(ClinicalChatPermission.VIEW)
