...
export class BuildPatientContextService {
  ...
  async execute(input: BuildPatientContextInput): Promise<PatientContextOutput> {
    ...
    const [clinicalProfile, activeAlerts, recentRecordsResult, recentFormsResult] =
      await Promise.all([
  ...
  private async mapFormsForChunking(
    forms: PatientForm[],
  ): Promise<PatientContextOutput["relevantForms"]> {
    ...
    for (const form of forms) {
      ...
      result.push({
        ...
        indexedFields: indexedFields.map((f) => ({
          ...
          value:
            f.valueText ?? f.valueNumber ?? f.valueBoolean ?? f.valueDate ?? f.valueJson ?? null,
        })),
      });
    }
