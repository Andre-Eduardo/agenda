...
function makeRankedChunk(content: string, score: number, sourceType = ContextChunkSourceType.RECORD): RankedChunk {
    return {
...
function makeService(overrides?: {
    ...
    const snapshotRepository = overrides?.snapshotRepository ?? mock<PatientContextSnapshotRepository>();
...
describe('RetrievePatientChunksService — edge cases', () => {
    ...
    it('calls searchSimilar with limit 0 when topK is 0', async () => {
        ...
        expect(chunkRepository.searchSimilar).toHaveBeenCalledWith(expect.objectContaining({limit: 0}));
    });
    ...
    it('propagates repository error when pgvector dimension does not match', async () => {
        ...
        chunkRepository.searchSimilar.mockRejectedValue(new Error('wrong number of dimensions 512 should be 1536'));
