...
import {ContextPolicyService} from '@application/clinical-chat/services/context-policy.service';
import {GetContextSnapshotService} from '@application/clinical-chat/services/get-context-snapshot.service';
import {RetrievePatientChunksService} from '@application/clinical-chat/services/retrieve-patient-chunks.service';
 ...
 */
@Injectable()
export class SendChatMessageService implements ApplicationService<SendChatMessageInput, SendChatMessageOutput> {
    ...
    async execute({payload, actor}: Command<SendChatMessageInput>): Promise<SendChatMessageOutput> {
        ...
        const policyFilteredChunks = this.contextPolicyService.applyToChunks(retrievalResult.chunks, effectivePolicy);
        ...
        const recentMessagesResult = await this.messageRepository.listBySession(payload.sessionId, {
        ...
        const sanitizedFacts = snapshot?.patientFacts
            ? this.contextPolicyService.sanitizePatientFacts(snapshot.patientFacts, effectivePolicy.blacklistedFields)
            : null;
        ...
        const resolvedModel =
            agentProfile?.providerModelId ?? getDefaultModelForSpecialty(agentProfile?.specialtyGroup ?? null);

        const provider = this.aiProviderRegistry.getChatProvider();
        const useAgentTools = process.env.AGENT_ENABLE_TOOLS === 'true' && this.agentLoop != null;
        ...
        try {
            ...
            if (
                ...
                if (costUsd === null) {
                    ...
                    this.interactionLogRepository.save(interactionLog).catch((error: unknown) => {
    ...
    private buildMessagePayload(params: {
        ...
        if (params.timelineSummary && params.timelineSummary.length > 0) {
            const timelineLines = params.timelineSummary
                ...
                .map(
                    (t) =>
                        `- [${t.date.split('T')[0]}] ${t.title ?? t.type}${t.summary ? `: ${t.summary.slice(0, 120)}` : ''}`
                )
                .join('\n');
