 ...
 */
@Injectable()
export class SendChatMessageService implements ApplicationService<
...
> {
  ...
  private buildMessagePayload(params: {
    ...
    if (params.patientFacts) {
      ...
      if (facts.age !== null && facts.age !== undefined)
        factsLines.push(`Idade: ${facts.age} anos`);

      if (facts.gender) factsLines.push(`Gênero: ${facts.gender}`);

      if (facts.allergies) factsLines.push(`Alergias: ${facts.allergies}`);

      if (facts.chronicConditions)
        factsLines.push(`Condições crônicas: ${facts.chronicConditions}`);

      if (facts.currentMedications)
        factsLines.push(`Medicamentos em uso: ${facts.currentMedications}`);
