...
export class AgentResolutionService {
    ...
    async resolveForMember(memberId: ClinicMemberId): Promise<AiAgentProfile> {
        ...
        const specialtyText = professional.specialty ? normalizeSpecialtyText(professional.specialty) : null;

        const activeRules = AGENT_MAPPING_RULES.filter((r) => r.isActive).toSorted((a, b) => b.priority - a.priority);
        ...
        throw new PreconditionException("No AI agent matched the member's profile and no generic agent is configured.");
    }
