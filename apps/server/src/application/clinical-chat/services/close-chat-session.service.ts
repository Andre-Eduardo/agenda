...
@Injectable()
export class CloseChatSessionService
    implements ApplicationService<CloseChatSessionInput, PatientChatSession>
{
    ...
    async execute({payload}: Command<CloseChatSessionInput>): Promise<PatientChatSession> {
        ...
        if (session.status === ChatSessionStatus.ACTIVE) {
            payload.archive ? session.archive() : session.close();
