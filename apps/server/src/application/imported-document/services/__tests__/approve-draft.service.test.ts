...
describe('ApproveDraftService', () => {
    ...
    it('should approve the draft, create a Record, and advance the document to APPROVED', async () => {
        ...
        professionalRepository.findByClinicMemberId.mockResolvedValueOnce(
            fakeProfessional(clinicMemberId) as Professional
        );
        ...
        expect(draft.events).toContainEqual(expect.objectContaining({type: DraftApprovedEvent.type}));
    });
    ...
    it('should throw ResourceNotFoundException when the ImportedDocument is not found', async () => {
        ...
        await expect(service.execute({actor, payload: {id: ImportedDocumentId.generate()}})).rejects.toThrowWithMessage(
            ResourceNotFoundException,
            'ImportedDocument not found.'
        );
    });
    ...
    it('should throw ResourceNotFoundException when the draft is not found', async () => {
        ...
        await expect(service.execute({actor, payload: {id: document.id}})).rejects.toThrowWithMessage(
            ResourceNotFoundException,
            'Draft not found for this document.'
        );
    });
    ...
    it('should throw PreconditionException when the actor has no Professional record', async () => {
        ...
        await expect(service.execute({actor, payload: {id: document.id}})).rejects.toThrowWithMessage(
            ...
            'DRAFT_APPROVAL_REQUIRES_PROFESSIONAL'
        );
