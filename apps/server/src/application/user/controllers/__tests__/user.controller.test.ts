...
describe('A user controller', () => {
    ...
    describe('when signing up a user', () => {
        it('should repass the responsibility to the right service', async () => {
            ...
            await expect(userController.signUp(unauthenticatedActor, payload)).resolves.toEqual(expectedUser);

            expect(signUpUserServiceMock.execute).toHaveBeenCalledWith({actor: unauthenticatedActor, payload});
        });
    });
    ...
    describe('when getting a user', () => {
        it('should repass the responsibility to the right service', async () => {
            ...
            expect(getUserServiceMock.execute).toHaveBeenCalledWith({actor, payload: {id: existingUser.id}});
        });
    });
    ...
    describe('when updating a user', () => {
        it('should repass the responsibility to the right service', async () => {
            ...
            await expect(userController.updateUser(actor, existingUser.id, payload)).resolves.toEqual(expectedUser);
    ...
    describe('when deleting a user', () => {
        it('should repass the responsibility to the right service', async () => {
            ...
            expect(deleteUserServiceMock.execute).toHaveBeenCalledWith({actor, payload: {id, ...payload}});
        });
    });
});
