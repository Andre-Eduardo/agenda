...
import { FormScoringService } from "@application/patient-form/services/form-scoring.service";
import { FormFieldIndexerService } from "@application/patient-form/services/form-field-indexer.service";
import { CompletePatientFormDto, PatientFormDto } from "@application/patient-form/dtos";
import { inferSpecialtyGroup } from "@domain/form-template/entities";

@Injectable()
export class CompletePatientFormService implements ApplicationService<
  CompletePatientFormDto,
  PatientFormDto
> {
  constructor(
  ...
  async execute({
    actor: _actor,
    payload,
  }: Command<CompletePatientFormDto>): Promise<PatientFormDto> {
