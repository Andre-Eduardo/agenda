...
export class BuildRecordFromPatientFormService {
    ...
    async execute(patientFormId: PatientFormId): Promise<RecordPayloadFromForm> {
        ...
        const plan = form.computedJson?.flags?.length ? `Flags: ${form.computedJson.flags.join(', ')}` : undefined;
    ...
    private buildFreeNotes(
        ...
        const fieldLabelMap = new Map(definition.sections.flatMap((s) => s.fields.map((f) => [f.id, f.label])));
