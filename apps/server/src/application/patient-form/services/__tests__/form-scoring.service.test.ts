        ...
        rules: [
            {
                id: 'rule_low',
                label: 'Low',
                ranges: [{min: 0, max: 2, classification: 'Low', flag: 'LOW_RISK'}],
            },
            {
                id: 'rule_high',
                label: 'High',
                ranges: [{min: 3, max: 10, classification: 'High', flag: 'HIGH_RISK'}],
            },
        ],
    },
};
...
describe('FormScoringService', () => {
    ...
    it('should return scores array with per-field entries', () => {
        const result = service.compute(definitionWithScoring, [{fieldId: 'q1', valueText: 'sometimes'}]);
