...
import type {
  ...
  FormFieldDefinition,
  FormAnswer,
} from "@domain/form-template/types";
    ...
    switch (field.type) {
      ...
      case "textarea": {
        ...
        if (
          ...
          answer.valueText.length < field.validation.minLength
        ) {
          violations.push({
            field: field.id,
        ...
        if (
          ...
          answer.valueText.length > field.validation.maxLength
        ) {
          violations.push({
            field: field.id,
      ...
      case "computed":
      case "file": {
        // No additional validation for computed or file fields
        break;
      }
    }
