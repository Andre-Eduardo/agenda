...
export class FormAiContextService {
  ...
  async buildTextTimeline(patientId: PatientId, filter: AiContextFilter = {}): Promise<string> {
    ...
    for (const form of context.forms) {
      ...
      for (const field of form.indexedFields) {
        const val =
          field.value !== null && field.value !== undefined ? JSON.stringify(field.value) : "—";
  ...
  private mapIndexedFields(entries: FormFieldIndex[]): AiFormEntry["indexedFields"] {
    return entries.map((e) => ({
      fieldId: e.fieldId,
      fieldLabel: e.fieldLabel,
      fieldType: e.fieldType,
      value: e.valueText ?? e.valueNumber ?? e.valueBoolean ?? e.valueDate ?? e.valueJson ?? null,
    }));
  }
}
