...
import {PrepareUploadInputDto, PreparedUploadDto, TempUploadDto, UploadLocalInputDto} from '@application/upload/dtos';
...
export class UploadController {
    ...
    @Post('prepare')
    prepareUpload(@RequestActor() actor: Actor, @Body() payload: PrepareUploadInputDto): Promise<PreparedUploadDto> {
    ...
    uploadLocal(@UploadedFile() file: Express.Multer.File, @Body() payload: UploadLocalInputDto): TempUploadDto {
        if (this.config.storage.type !== FileStorageType.LOCAL) {
            throw new InvalidInputException('Local upload is not available with the current storage configuration.');
        }
