...
import type { ApplicationService, Command } from "@application/@shared/application.service";
import type { PrepareUploadDto } from "@application/upload/dtos/prepare-upload.dto";
import type { PreparedUploadDto } from "@application/upload/dtos/prepared-upload.dto";
import { UploadExceptions } from "@application/upload/pipes/file-validation.pipe";

@Injectable()
export class PrepareUploadService implements ApplicationService<
  PrepareUploadDto,
  PreparedUploadDto
> {
  constructor(
  ...
  async execute({ payload }: Command<PrepareUploadDto>): Promise<PreparedUploadDto> {
    ...
    const upload = await this.fileStorage.prepareUpload(
      tempPath,
      payload.mimeType,
      payload.checksum,
    );
