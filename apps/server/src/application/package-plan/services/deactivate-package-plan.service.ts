...
import {DeactivatePackagePlanDto, PackagePlanDto} from '@application/package-plan/dtos';
import {PreconditionException, ResourceNotFoundException} from '@domain/@shared/exceptions';
import {EventDispatcher} from '@domain/event';
import {PackagePlanRepository} from '@domain/package-plan/package-plan.repository';

@Injectable()
export class DeactivatePackagePlanService implements ApplicationService<DeactivatePackagePlanDto, PackagePlanDto> {
    constructor(
        private readonly packagePlanRepository: PackagePlanRepository,
        private readonly eventDispatcher: EventDispatcher
    ) {}

    async execute({actor, payload: {id}}: Command<DeactivatePackagePlanDto>): Promise<PackagePlanDto> {
