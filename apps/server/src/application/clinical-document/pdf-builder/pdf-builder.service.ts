...
export class PdfBuilderService {
  generateBuffer(ctx: PdfBuildContext): Promise<Buffer> {
    const docDefinition = this.buildDocDefinition(ctx);
    // eslint-disable-next-line @typescript-eslint/no-require-imports -- pdfmake does not support ESM
    const pdfmake = require("pdfmake/build/pdfmake") as typeof import("pdfmake");
    // eslint-disable-next-line @typescript-eslint/no-require-imports -- pdfmake font loaded via CommonJS
    const HelveticaFont = require("pdfmake/build/standard-fonts/Helvetica");

    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument -- pdfmake font container loaded via CommonJS require
  ...
  private buildDocDefinition(ctx: PdfBuildContext): TDocumentDefinitions {
    ...
    const header: Content = {
      stack: [
        ...
        {
          canvas: [
            { type: "line", x1: 0, y1: 5, x2: 515, y2: 5, lineWidth: 1, lineColor: "#cccccc" },
          ],
        },
      ],
      margin: [40, 20, 40, 10],
  ...
  private buildContentSection(document: ClinicalDocument): Content {
    switch (document.type) {
      ...
      default: {
        throw new Error(`Unsupported document type: ${String(document.type)}`);
      }
    }
  }
  ...
  private buildPrescriptionContent(content: PrescriptionContent): Content {
    ...
    const medicationRows = content.medications.map((med, idx) => ({
      stack: [
        ...
        {
          text: `${med.dosage} — ${med.frequency} por ${med.duration}`,
  ...
  private buildCertificateContent(content: MedicalCertificateContent): Content {
    ...
    const startDate = new Date(Number(year), Number(month) - 1, Number(day)).toLocaleDateString(
      "pt-BR",
    );
    ...
    return {
      stack: [
        ...
        content.cid
          ? { text: `CID-10: ${content.cid}`, style: "bodyText", margin: [0, 8, 0, 0] }
          : "",
        content.observations
          ? { text: `Observações: ${content.observations}`, style: "observations" }
          : "",
      ...
      stack: [
        { text: "Encaminho para avaliação e conduta:", style: "sectionHeader" },
        {
          table: {
            ...
            body: [
              [
                { text: "Especialidade:", bold: true, style: "bodyText" },
                { text: content.specialty, style: "bodyText" },
              ],
              [
                { text: "Motivo:", bold: true, style: "bodyText" },
                { text: content.reason, style: "bodyText" },
              ],
              [
                { text: "Prioridade:", bold: true, style: "bodyText" },
                { text: urgencyLabel[content.urgency] ?? content.urgency, style: "bodyText" },
              ],
            ],
          },
  ...
  private buildExamRequestContent(content: ExamRequestContent): Content {
    ...
    return {
      stack: [
        ...
        ...examRows,
        content.priority
          ? { text: `Prioridade: ${content.priority}`, style: "bodyText", margin: [0, 8, 0, 0] }
          : "",
        content.observations
          ? { text: `Observações: ${content.observations}`, style: "observations" }
          : "",
