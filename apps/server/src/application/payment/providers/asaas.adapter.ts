...
export class AsaasPaymentAdapter implements IPaymentProvider {
    ...
    constructor(private readonly configService: EnvConfigService) {
        ...
        this.apiKey = apiKey;
        this.baseUrl = env === 'production' ? 'https://api.asaas.com/v3' : 'https://sandbox.asaas.com/api/v3';
    }
    ...
    async getSubscriptionStatus(externalSubscriptionId: string): Promise<ExternalSubscriptionStatus> {
        const result = await this.request<AsaasSubscriptionResponse>(`/subscriptions/${externalSubscriptionId}`);
