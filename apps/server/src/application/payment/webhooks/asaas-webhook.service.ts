...
export class AsaasWebhookService {
    ...
    async handleWebhook(rawPayload: unknown): Promise<void> {
        ...
        try {
            await this.processEvent(event, subscription.memberId, subscription.clinicId, subscription.id, payment, now);
    ...
    private async processEvent(
        ...
        switch (event) {
            ...
            case 'PAYMENT_RECEIVED': {
                ...
                await this.paymentService.createUsageRecordForNextPeriodIfNeeded(subscriptionId, memberId, clinicId);
            ...
            case 'PAYMENT_OVERDUE': {
                ...
                if (suspended) {
                    this.logger.warn(`Subscription suspended for member ${memberId} — overdue ${daysPastDue} days`);
                }
