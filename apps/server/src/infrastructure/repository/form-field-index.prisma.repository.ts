...
import {
  FormFieldIndexRepository,
  FormFieldIndexFilter,
} from "@domain/form-field-index/form-field-index.repository";
...
@Injectable()
export class FormFieldIndexPrismaRepository
  extends PrismaRepository
  implements FormFieldIndexRepository
{
  constructor(
  ...
  async search(filter: FormFieldIndexFilter): Promise<FormFieldIndex[]> {
    const where: PrismaClient.Prisma.FormFieldIndexWhereInput = {
      ...
      specialtyGroup:
        toEnumOrNull(PrismaClient.AiSpecialtyGroup, filter.specialtyGroup) ?? undefined,
    };
