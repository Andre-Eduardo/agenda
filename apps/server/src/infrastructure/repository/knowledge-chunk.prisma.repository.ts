...
@Injectable()
export class KnowledgeChunkPrismaRepository extends PrismaRepository implements KnowledgeChunkRepository {
    constructor(
    ...
    async searchSimilar(filter: KnowledgeChunkSearchFilter): Promise<RankedKnowledgeChunk[]> {
        ...
        const categoryClause = filter.category ? Prisma.sql`AND category = ${filter.category}` : Prisma.empty;
