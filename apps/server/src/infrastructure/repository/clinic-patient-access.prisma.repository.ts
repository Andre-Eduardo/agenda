...
@Injectable()
export class ClinicPatientAccessPrismaRepository extends PrismaRepository implements ClinicPatientAccessRepository {
    constructor(
    ...
    async findByMemberAndPatient(memberId: ClinicMemberId, patientId: PatientId): Promise<ClinicPatientAccess | null> {
        const access = await this.prisma.clinicPatientAccess.findFirst({
            where: {
