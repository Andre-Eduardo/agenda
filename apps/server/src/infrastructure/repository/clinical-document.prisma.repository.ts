...
import {ClinicalDocumentMapper} from '../mappers/clinical-document.mapper';
import {PrismaProvider} from './prisma/prisma.provider';
import {PrismaRepository} from './prisma.repository';

@Injectable()
export class ClinicalDocumentPrismaRepository extends PrismaRepository implements ClinicalDocumentRepository {
    constructor(
    ...
    async saveWithGeneratedFile(document: ClinicalDocument, fileData: GeneratedFileData): Promise<string> {
        ...
        await this.prismaProvider.runAtomically(async () => {
            ...
            await this.prisma.clinicalDocument.upsert({
                where: {id: documentData.id as string},
                create: documentData,
                update: documentData,
            });
        });
