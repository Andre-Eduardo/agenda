...
import {UserProfessionalAddedEvent, UserProfessionalRemovedEvent} from '../../../domain/user/events';
...
describe('EventPrismaRepository', () => {
    ...
    let user: User;
    let events: Array<{timestamp: Date; type: string; [key: string]: unknown}>;
    ...
    beforeAll(async () => {
        user = await User.create({
            ...
            password: 'Pa$$w0rd' as unknown as import('../../../domain/user/value-objects').ObfuscatedPassword,
            name: 'John Doe',
        ...
        databaseEvents = [
            {
                ...
                } as unknown as PrismaClient.Prisma.JsonValue,
                userIp: '127.0.0.1',
    ...
    it('should search events', async () => {
        ...
        prisma.event.findMany.mockResolvedValueOnce(databaseEvents);
        prisma.event.count.mockResolvedValueOnce(databaseEvents.length);
