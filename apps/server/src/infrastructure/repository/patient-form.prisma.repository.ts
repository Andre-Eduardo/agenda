...
import {
  PatientFormRepository,
  PatientFormFilter,
  PatientFormSortOptions,
} from "@domain/patient-form/patient-form.repository";
...
export class PatientFormPrismaRepository extends PrismaRepository implements PatientFormRepository {
  ...
  async save(form: PatientForm): Promise<void> {
    ...
    const writeData: Prisma.PatientFormUncheckedCreateInput = {
      ...
      computedJson:
        data.computedJson === null ? Prisma.JsonNull : (data.computedJson as Prisma.InputJsonValue),
    };
  ...
  async search(
    ...
    const where: PrismaClient.Prisma.PatientFormWhereInput = {
      ...
      template: filter.specialty
        ? {
            specialtyGroup:
              toEnumOrNull(PrismaClient.AiSpecialtyGroup, filter.specialty) ?? undefined,
          }
        : undefined,
    };
