...
export class S3FileStorage implements FileStorage {
    ...
    async getFileInfo(filePath: string): Promise<{size: number; checksum: string} | null> {
        try {
            const response = await this.client.send(new HeadObjectCommand({Bucket: this.bucket, Key: filePath}));

            const size = response.ContentLength ?? 0;
            const checksum = response.ChecksumSHA256 ?? response.ETag?.replaceAll('"', '') ?? '';
