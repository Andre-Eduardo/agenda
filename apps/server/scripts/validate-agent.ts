...
function post(url: string, body: unknown, cookieHeader: string): Promise<{status: number; body: unknown}> {
...
async function main() {
    ...
    const questions = CATEGORY_FILTER ? allQuestions.filter((q) => q.category === CATEGORY_FILTER) : allQuestions;
    ...
    for (const q of questions) {
        ...
        try {
            const response = await post(`${BASE_URL}/api/v1/agent/ask`, {message: q.question}, SESSION_TOKEN);
            ...
            const matchedKeywords = q.expectedKeywords.filter((kw) => answerLower.includes(kw.toLowerCase()));
            const missedKeywords = q.expectedKeywords.filter((kw) => !answerLower.includes(kw.toLowerCase()));
