...
- **State-within-Record**: Using `wasHumanEdited=false` + a special status field on `Record` would mean the draft _is already a Record_ — violating the requirement that AI content must not become a Record without approval.
