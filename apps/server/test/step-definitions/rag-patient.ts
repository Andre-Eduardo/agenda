 ...
 */
Given('um profissional logado com especialidade {string}', async function (this: Context, specialty: string) {
    ...
    chai.expect(userResp.status, `User creation failed: ${JSON.stringify(userResp.body)}`).to.equal(201);
    ...
    chai.expect(signInResp.status, `Sign-in failed: ${JSON.stringify(signInResp.body)}`).to.equal(200);
    ...
    chai.expect(profResp.status, `Professional creation failed: ${JSON.stringify(profResp.body)}`).to.equal(201);
    ...
    chai.expect(signInProfResp.status, `Professional sign-in failed: ${JSON.stringify(signInProfResp.body)}`).to.equal(
        200
    );
});
 ...
 */
Given('um paciente {string} com evolução {string}', async function (this: Context, name: string, evolution: string) {
    ...
    const rebuildResp = await this.agent.post('/api/v1/clinical-chat/context/rebuild').send({patientId, reindex: true});
...
Given('os chunks do paciente foram indexados', async function (this: Context) {
    ...
    chai.expect(rebuildResp.status, `Re-index failed: ${JSON.stringify(rebuildResp.body)}`).to.equal(200);
});
...
When('consulto chunks com query {string}', async function (this: Context, query: string) {
    ...
    chai.expect(response.status, `Chunk retrieval failed: ${JSON.stringify(response.body)}`).to.equal(200);
...
When(
    ...
    async function (this: Context, name: string, query: string) {
        ...
        chai.expect(response.status, `Chunk retrieval failed for "${name}": ${JSON.stringify(response.body)}`).to.equal(
            200
        );
...
Then('o primeiro chunk tem score > {float}', function (this: Context, minScore: number) {
    ...
    chai.expect(chunks[0].score, `Expected first chunk score ${chunks[0].score} to be > ${minScore}`).to.be.greaterThan(
        minScore
    );
});
