...
export type VariableIdType =
  ...
  | "form-template"
  | "patient-form"
  // eslint-disable-next-line @typescript-eslint/ban-types -- allows literal type hints while keeping string extensibility
  | (string & {});
...
export class Context extends World {
  ...
  public getVariableId(idType: VariableIdType, key: string): string {
    ...
    if (id === undefined) {
      throw new Error(
        `Variable ID not found: [${idType}] "${key}". Available: ${Object.keys(this.variables.ids[idType] ?? {}).join(", ")}`,
      );
    }
