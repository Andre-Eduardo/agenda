Feature: Start blockade
    As a user, I want to start a blockade in the room to indicate that the room is not available for use.

    Background:
        Given a company with name "Ecxus" exists
        And the following employees with system access in the company "Ecxus" exist:
            | User       | Permissions      |
            | john_doe   | [blockade:start] |
            | william123 | []               |
    ...
    Scenario: Preventing unauthorized users from starting a blockade
        Given I am signed in as "william123" in the company "Ecxus"
        When I send a "POST" request to "/blockade" with:
            | roomId  | ${ref:id:room:Ecxus:1}            |
            | note    | "Defect in the room"              |
            | defects | ["${ref:id:defect:Ecxus:1:Door}"] |
        Then I should receive an access denied error with reason INSUFFICIENT_PERMISSIONS

    Scenario: Starting a blockade
        ...
        And the following events in the company "Ecxus" should be recorded:
            ...
            | BLOCKADE_STARTED   | "2024-01-01T01:00:00Z" | "${ref:id:user:john_doe}" |
            | ROOM_STATE_CHANGED | "2024-01-01T01:00:00Z" | "${ref:id:user:john_doe}" |

    Scenario: Starting a blockade with finished defects
    ...
    Scenario: Starting a blockade in a room that is already blocked
        Given I am signed in as "john_doe" in the company "Ecxus"
        ...
        Then I should receive a precondition failed error with message "There is already blockade in this room."
