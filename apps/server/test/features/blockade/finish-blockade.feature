Feature: Finish blockade
    As a user, I want to finish a blockade in the room to indicate that the room can be used again.

    Background:
        Given a company with name "Ecxus" exists
        And the following employees with system access in the company "Ecxus" exist:
            | User       | Permissions       |
            | john_doe   | [blockade:finish] |
            | william123 | []                |
    ...
    Scenario: Preventing unauthorized users from finish a blockade
        Given I am signed in as "william123" in the company "Ecxus"
        When I send a "PATCH" request to "/blockade/room/${ref:id:room:Ecxus:1}/finish"
        Then I should receive an access denied error with reason INSUFFICIENT_PERMISSIONS

    Scenario: Finishing a blockade
        ...
        And the following events in the company "Ecxus" should be recorded:
            ...
            | BLOCKADE_FINISHED  | "2024-01-06T04:00:00Z" | "${ref:id:user:john_doe}" |
            | ROOM_STATE_CHANGED | "2024-01-06T04:00:00Z" | "${ref:id:user:john_doe}" |

    Scenario: Finishing a blockade that does not exist
        Given I am signed in as "john_doe" in the company "Ecxus"
        And "unknown-id" is defined as:
            """
            7f3c4fbe-bbb4-4c32-8cce-a68f3c992839
            """
        When I send a "PATCH" request to "/blockade/room/${ref:var:unknown-id}/finish"
        Then I should receive a resource not found error on "${ref:var:unknown-id}" with reason "No Blockade was found in the room"
