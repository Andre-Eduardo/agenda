Feature: View cleaning
    As a user, I want to view the cleaning and its details
    ...
    Background:
        ...
        And the following cleanings exist in the company "Nortec":
            | Room | Start user ID           | Created at           | Started at           | End reason | Finished at          | Updated at           | End user ID             |
    ...
    Scenario: Preventing unauthorized users from viewing cleanings
        Given I am signed in as "william123" in the company "Ecxus"
        When I send a "GET" request to "/cleaning"
        Then I should receive an access denied error with reason INSUFFICIENT_PERMISSIONS

    Scenario: Viewing cleanings
        Given I am signed in as "john-doe" in the company "Nortec"
        When I send a "GET" request to "/cleaning" with the query:
            | pagination.limit | 10 |
    ...
    Scenario: Viewing a cleaning
        Given I am signed in as "john-doe" in the company "Nortec"
        When I send a "GET" request to "/cleaning/${ref:id:cleaning:Nortec:15}"
        Then the request should succeed with a 200 status code
        And the response should match:
            """JSON
            {
    ...
    Scenario: Viewing a cleaning by room
        Given I am signed in as "john-doe" in the company "Ecxus"
        When I send a "GET" request to "/cleaning/room/${ref:id:room:Ecxus:1}"
        Then the request should succeed with a 200 status code
        And the response should match:
            """JSON
            {
    ...
    Scenario: Viewing a cleaning by room that does not exist
        Given I am signed in as "john-doe" in the company "Ecxus"
        When I send a "GET" request to "/cleaning/room/${ref:id:room:Ecxus:3}"
        Then I should receive a resource not found error on "${ref:id:room:Ecxus:3}" with reason "Cleaning not found in room"

    Scenario: Viewing a cleaning that does not exist
        Given I am signed in as "john-doe" in the company "Ecxus"
        And "unknown-id" is defined as:
            """
            573a3ac3-a05a-48aa-80b1-1869a73bfe8c
            """
        When I send a "GET" request to "/cleaning/${ref:var:unknown-id}"
