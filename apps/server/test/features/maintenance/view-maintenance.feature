Feature: View maintenance
    As a user, I want to view the maintenance and its details
    ...
    Background:
        ...
        And the following maintenances exist in the company "Nortec":
    ...
    Scenario: Preventing unauthorized users from viewing maintenances
        Given I am signed in as "william123" in the company "Ecxus"
        When I send a "GET" request to "/maintenance"
        Then I should receive an access denied error with reason INSUFFICIENT_PERMISSIONS

    Scenario: Viewing maintenances
        Given I am signed in as "john_doe" in the company "Nortec"
        When I send a "GET" request to "/maintenance" with the query:
            | pagination.limit | 10 |
        ...
        And the response should match:
            """JSON
            {
                "data": [
                    {
                        "id": "${ref:id:maintenance:Nortec:15}",
                    ...
                    {
                        "id": "${ref:id:maintenance:Nortec:16}",
                        "companyId": "${ref:id:company:Nortec}",
    ...
    Scenario: Filtering maintenances
        Given I am signed in as "john_doe" in the company "Nortec"
        When I send a "GET" request to "/maintenance" with the query:
            | roomId           | "${ref:id:room:Nortec:15}" |
            | pagination.limit | 10                         |
    ...
    Scenario: Paginating maintenances
        Given I am signed in as "john_doe" in the company "Nortec"
        When I send a "GET" request to "/maintenance" with the query:
            | pagination.limit | 1                    |
            | pagination.sort  | {"createdAt": "asc"} |
    ...
    Scenario: Viewing a maintenance
        Given I am signed in as "john" in the company "Ecxus"
        When I send a "GET" request to "/maintenance/${ref:id:maintenance:Ecxus:1}"
        Then the request should succeed with a 200 status code
        And the response should match:
            """JSON
            {
    ...
    Scenario: Viewing an open maintenance of a room
        Given I am signed in as "john" in the company "Ecxus"
        When I send a "GET" request to "/maintenance/room/${ref:id:room:Ecxus:1}"
        Then the request should succeed with a 200 status code
        And the response should match:
            """JSON
            {
    ...
    Scenario: Viewing a maintenance that does not exist
        Given I am signed in as "john" in the company "Ecxus"
        And "unknown-id" is defined as:
            """
            573a3ac3-a05a-48aa-80b1-1869a73bfe8c
            """
        When I send a "GET" request to "/maintenance/${ref:var:unknown-id}"
        Then I should receive a resource not found error on "${ref:var:unknown-id}" with reason "Maintenance not found"

    Scenario: Viewing a non-existent maintenance of a room
        Given I am signed in as "john" in the company "Ecxus"
        And "unknown-id" is defined as:
            """
            573a3ac3-a05a-48aa-80b1-1869a73bfe8c
            """
        When I send a "GET" request to "/maintenance/room/${ref:var:unknown-id}"
        Then I should receive a resource not found error on "${ref:var:unknown-id}" with reason "No maintenance found for the room."
