Feature: Patient alert deletion (DELETE)
    As an authenticated professional I want to soft-delete clinical alerts for a patient
    so that inactive warnings no longer surface before encounters.
